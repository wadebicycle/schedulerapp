          ...
          <CardContent className="p-0">
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-black uppercase tracking-[0.25em]">Kéo di chuyển</span>
              </div>
              <button
                type="button"
...
export default function App() {
  ...
  const handleUpdatePlan = async (updatedPlan: Plan) => {
    ...
    if (user) {
      ...
      sessionStorage.setItem('chronos_excel_plans', JSON.stringify(nextPlans));
      toast.success('Đã cập nhật công việc (session)');
    }
  };
  ...
  const handleDeletePlan = async (id: string) => {
    ...
    if (user) {
      ...
      sessionStorage.setItem('chronos_excel_plans', JSON.stringify(nextPlans));
      toast.info('Đã xóa công việc (session)');
    }
  };
  ...
  return (
    <div className={cn(
      ...
      {motivationMessage && (
        <div className="fixed bottom-20 right-4 z-50 pointer-events-none max-w-xs rounded-2xl border border-[#107C41] bg-slate-950/95 p-4 shadow-2xl text-white backdrop-blur-sm">
          <div className="flex items-center gap-2">
            <div className="flex h-9 w-9 items-center justify-center rounded-full bg-[#107C41] text-white">
              <CheckCircle2 className="w-4 h-4" />
            </div>
            <div className="text-sm font-semibold leading-snug">{motivationMessage}</div>
          </div>
        </div>
      )}
